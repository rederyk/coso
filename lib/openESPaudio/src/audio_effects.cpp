...
EqualizerEffect::EqualizerEffect() = default;

float EqualizerEffect::getBandGain(Band band) const {
    return band_gains_[static_cast<int>(band)];
}

void EqualizerEffect::setBandGain(Band band, float gain) {
    band_gains_[static_cast<int>(band)] = gain;
}

void EqualizerEffect::applyPreset(const std::string& preset_name) {
    if (preset_name == "Flat") {
        std::fill_n(band_gains_, 5, 0.0f);
    } else if (preset_name == "Rock") {
        band_gains_[0] = 3.0f;  // Bass
        band_gains_[1] = 1.0f;  // Low-Mid
        band_gains_[2] = 0.0f;  // Mid
        band_gains_[3] = 2.0f;  // High-Mid
        band_gains_[4] = 3.0f;  // Treble
    } else if (preset_name == "Jazz") {
        band_gains_[0] = 2.0f;  // Bass
        band_gains_[1] = -1.0f; // Low-Mid
        band_gains_[2] = 1.0f;  // Mid
        band_gains_[3] = 2.0f;  // High-Mid
        band_gains_[4] = 1.0f;  // Treble
    } else if (preset_name == "Classical") {
        band_gains_[0] = 2.0f;  // Bass
        band_gains_[1] = 0.0f;  // Low-Mid
        band_gains_[2] = 0.0f;  // Mid
        band_gains_[3] = 0.0f;  // High-Mid
        band_gains_[4] = 2.0f;  // Treble
    } else if (preset_name == "Pop") {
        band_gains_[0] = -1.0f; // Bass
        band_gains_[1] = 2.0f;  // Low-Mid
        band_gains_[2] = 3.0f;  // Mid
        band_gains_[3] = 1.0f;  // High-Mid
        band_gains_[4] = -1.0f; // Treble
    } else if (preset_name == "Bass Boost") {
        band_gains_[0] = 5.0f;  // Bass
        band_gains_[1] = 2.0f;  // Low-Mid
        band_gains_[2] = 0.0f;  // Mid
        band_gains_[3] = -1.0f; // High-Mid
        band_gains_[4] = -2.0f; // Treble
    } else if (preset_name == "Treble Boost") {
        band_gains_[0] = -2.0f; // Bass
        band_gains_[1] = -1.0f; // Low-Mid
        band_gains_[2] = 0.0f;  // Mid
        band_gains_[3] = 2.0f;  // High-Mid
        band_gains_[4] = 5.0f;  // Treble
    }
}
