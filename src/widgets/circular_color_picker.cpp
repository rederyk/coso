...
lv_obj_t* CircularColorPicker::create(lv_obj_t* parent, lv_coord_t size, uint8_t brightness) {
    ...
    size_t buffer_size = LV_CANVAS_BUF_SIZE_TRUE_COLOR(size, size);
    lv_color_t* cbuf = (lv_color_t*)heap_caps_malloc(buffer_size, MALLOC_CAP_SPIRAM);
    if (!cbuf) {
        Serial.println("❌ Failed to allocate canvas buffer");
        delete data;
        return nullptr;
    }
    lv_canvas_set_buffer(data->canvas, cbuf, size, size, LV_IMG_CF_TRUE_COLOR);
    ...
    for (lv_coord_t y = 0; y < size; y++) {
        for (lv_coord_t x = 0; x < size; x++) {
            ...
            if (dist <= radius) {
                ...
                uint16_t hue = (uint16_t)((angle + M_PI) * 180.0f / M_PI); // 0-360
...
void CircularColorPicker::update_cursor_position(lv_obj_t* obj) {
    ...
    lv_coord_t cursor_x = center + (lv_coord_t)(dist * cosf(angle)) - 6; // -6 for cursor centering
    lv_coord_t cursor_y = center + (lv_coord_t)(dist * sinf(angle)) - 6;
